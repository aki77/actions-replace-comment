...
export const findComment = async ({
  ...
  const comment = existingComments.find((comment) =>
    comment.body?.startsWith(firstLine),
  );
...
export const deleteComment = async ({
  ...
  body,
  startsWith = false,
}: Readonly<DeleteCommentOptions>): Promise<void> => {
  const { comment_id, exactMatch } = await findComment({
    token,
    owner,
    repo,
    issue_number,
...
export const createComment = async ({
  ...
  const responce = await issues(token).createComment({
    owner,
    repo,
    issue_number,
    body,
...
export default async function replaceComment({
  token,
  owner,
  repo,
  issue_number,
  ...
  undefined | CreateCommentResponse
> {
  const { comment_id, exactMatch } = await findComment({
    token,
    owner,
